...
exports.createEmailBulkTemplate = async () => {
  try {
    const rawdata = fs.readFileSync(
      path.resolve(__dirname, "../../utils/emails/email-template.json")
    );
    ...
    if (emailId) {
      ...
      if (email)
        ...
        return res.status(400).send({
          success: false,
          message: "Email template not found for given Id",
        });
    } else
      return res
        .status(400)
...
exports.delete = async (req, res, next) => {
  try {
    const { emailId } = req.params;
    if (emailId) {
      const { deletedCount } = await Email.deleteOne({ _id: emailId });
      if (deletedCount)
        return res.send({
          success: true,
        ...
        return res
          .status(400)
          .send({ success: false, message: "Email not found for given Id" });
    } else
      return res
        .status(400)
        .send({ success: false, message: "Email Id is required" });
